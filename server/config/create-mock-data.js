...
function createData () {
  ...
  let now = new Date('2018-01-28T10:30');
  ...
  Promise.all([usersPromise, roomsPromise, eventsPromise])
    ...
    .then(([users, rooms, events]) => {
      let promises = [];
      promises.push(events[0].setRoom(rooms[0].id));
      promises.push(events[1].setRoom(rooms[1]).id);
      promises.push(events[2].setRoom(rooms[2]).id);
